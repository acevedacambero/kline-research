...
def validate_top_score_portfolio(scores: pd.DataFrame | list[dict], labels: pd.DataFrame | list[dict], *, label_column: str = "p20_executable_return", top_fraction: float = 0.1, as_of_date: date | None = None, non_overlapping: bool = False) -> dict[str, Any]:
    ...
    max_drawdown = None
    if non_overlapping:
        period_returns = selected_returns.groupby(level=0).mean()
    ...
    return {**base, "sampleCount": int(len(merged)), "tradingDayCount": int(merged["date"].nunique()), "selectedCount": int(len(selected)), "averageReturn": float(selected_returns.mean()), "benchmarkReturn": float(benchmark.mean()), "excessReturn": float(selected_returns.mean() - benchmark.mean()), "winRate": float((selected_returns > 0).mean()), "maxDrawdown": max_drawdown, "nonOverlapping": non_overlapping, "warnings": warnings}
