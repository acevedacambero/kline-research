...
P1 标签、P2 特征和 P3 评分均与证券当前行情 `snapshotVersion` 严格绑定。行情更新后，状态接口会立即报告旧快照、缺失和孤儿产物；扫描、校准、训练、漂移监控和组合验证拒绝读取旧快照。页面“一键补齐 P1–P3”按依赖顺序运行三个阶段，支持持久化进度、中断续跑和只处理待重建证券。

页面“一键日常更新与研究维护”进一步把行情增量更新、全市场质量台账、P1、P2、P3 合并为同一个五阶段持久化任务。默认只重建行情快照发生变化或已有旧产物的证券，也支持全量运行研究层；P4–P8 仍由用户选择参数后单独执行。

系统运维状态接口统一报告磁盘余量、数据新鲜度、后台任务失败和 provider Gate；维护配置、数据源探测、备份提交与删除、产物清理和模型启用会写入不含密钥的持久化管理审计台账。

所有行情导入都会执行历史防缩短保护。若上游只返回近期窗口，系统会与现有历史按交易日合并，并记录 `history-shrink-prevented` 质量事件，不允许短窗口覆盖完整历史。

网页“自动维护与备份”提供衍生产物清理计划。扫描只纳入两类文件：不属于当前沪深证券清单的孤儿产物，以及已经存在当前快照替代品的旧产物；某只证券仅剩旧产物且尚未重建时不会清理。默认执行方式为隔离，完整计划和执行收据均持久化；API 也支持明确指定 `delete` 直接释放空间。

后台任务错误统一记录 `category` 和 `retryable`，类别包括网络、超时、数据结构及其他运行错误。任务面板汇总各类别数量和可重试数量，原始错误仍完整保留并可导出。

等待中或运行中的后台任务可以从任务进度卡或历史表请求“安全取消”。取消采用协作式检查点，不强杀进程；当前网络请求或文件写入完成后才停止。已取消的可恢复任务保留原任务 ID、参数和进度，再次续跑时从已落盘结果继续。

研究验收只统计与当前行情清单哈希一致的永久实验；旧数据快照实验会单独列出但不计入完成数。P7 模型保存同一快照哈希，激活模型若来自旧快照会成为验收阻断项，避免行情更新后继续把旧模型误认为当前模型。
...
- P6 优先使用依赖当前数据快照的受控当前模型排序并展示模型概率；当前模型缺失、过期或不可推理时明确退回 P3 分数排序。
...
- P8 组合验证同时展示交易成本、滑点、基准、净超额收益、最大回撤和相邻调仓的平均单向换手率。
